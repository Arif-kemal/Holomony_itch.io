...
//top nedir ne değildir
// top dürüm gibidir merkez noktası vardır, yarıçapı vardır, hareketi vardır, hızları vardır, yerde olup olmadığı vardır, zıplama sayacı vardır, squish (ezilme) efektleri vardır
const topumuz = {
...
function oyunuBaşlat() {
  ...
  // Müziği başlat ( tıklama sonrası çalışır)
  sesMüzik.volume = 0.4;
  sesMüzik.play().catch(() => { /* ses izni vermediyse sessiz devam */ });
...
// *********************************************
//  KLAVYE DİNLEYİCİLERİ-aynı java
...
function güncelle() {
  ...
  // yatay hareket
  if (tuşlar.sol)  topumuz.hizX -= HAREKET_HIZI * 0.4; if (tuşlar.sağ) topumuz.hizX += HAREKET_HIZI * 0.4;
  // Hız sınırı
  ...
  // yer çekimi foto gibi
  topumuz.hizY += YERÇEKIMI;
  topumuz.hizY  = Math.min(topumuz.hizY, MAX_DÜŞÜŞ_HIZI);

  // squish normalizasyonu her karede yavaşça 1.0'a geri döner
...
//  önce X sonra Y ekseninde ayrı ayrı çözERİZ
...
// Varsa bloğun sol/sağ kenar piksel değerlerini döndürür, yoksa null döndürür ya da iskender
...
//  Porta mı yoksa sağ-sol döngüsü mü? bakcez artık
...
//  Harita döndürülmez  top karşı kenardan girecek
...
//  Harita döndürülür, top hedef kenara yerleştirririz
...
// **************************
// SES YARDIMCI — Sesi baştan çalmalı hata varsa sessiz geçer beya
...
function topuÇiz() {
  const cx = topumuz.x;
  const cy = topumuz.y;
  const rx = topumuz.yariçap * topumuz.squishX; //yatay yarıçap 
  const ry = topumuz.yariçap * topumuz.squishY;   //dikey yarıçap 
